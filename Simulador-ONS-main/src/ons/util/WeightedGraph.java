...
public class WeightedGraph {
    ...
    public boolean isEdge(int source, int target) {
        return getEdges()[source][target] > 0;
    }
    ...
    public int[] neighbors(int vertex) {
        int count = 0;
        for (int i = 0; i < getEdges()[vertex].length; i++) {
    ...
    public void removeNode(int node) {
        ...
        double[][] newedges = new double[getNumNodes()-1][getNumNodes()-1];
        int k = 0, l = 0;
        for(int i = 0; i< getNumNodes(); i++){
            if(i != node){
                l = 0;
    ...
    public void addNode(){
        double[][] newedges = new double[getNumNodes()+1][getNumNodes()+1];
        for(int i = 0; i < getNumNodes(); i++){
            System.arraycopy(getEdges()[i], 0, newedges[i], 0, getNumNodes());
        }
        setNumNodes(getNumNodes() + 1);
        setEdges(newedges);
    }
    ...
    public double getAverageClusteringCoefficient(){
        return getSumClusteringCoefficient() / (double) this.getNumNodes();
    }
    ...
    public String toString() {
        ...
        for (int j = 0; j < getEdges().length; j++) {
            ...
            for (int i = 0; i < getEdges()[j].length; i++) {
                if (getEdges()[j][i] > 0) {
                    s += Integer.toString(i) + ":" + Double.toString(getEdges()[j][i]) + " ";
                }
            }
            s += "\n";
        }
