...
public class YenKSP {
    ...
    public static ArrayList<Integer>[] kDisruptedShortestPaths(WeightedGraph graph, int source, int destination, int K){
        ...
        for(int i=0; i<K; i++){
            ...
            int [] nodes = Dijkstra.getShortestPath(g, source, destination);
            ...
            for(int j=0; j<nodes.length-1;j++){
                ...
                g.setWeight(nodes[j+1], nodes[j], Integer.MAX_VALUE); 
                g.setWeight(nodes[j], nodes[j+1], Integer.MAX_VALUE);              
        ...
        ArrayList<Integer>[] pathsAux = new ArrayList[aux];
        aux = 0;
